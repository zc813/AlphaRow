...
def playworker(data_queue, param_queue):
    ...
    best_logic = ModelBasedMCTSLogic(models.new_model(input_shape,policy_width), iterations=500)
    latest_logic = ModelBasedMCTSLogic(models.new_model(input_shape,policy_width), iterations=500)
    eval_self_play.worker(game_player=game_player,
...
if __name__=='__main__':
    ...
    p = Process(target=playworker, args=(data_queue, param_queue))
    p.start()

    data = optimize.DataBuffer(input_shape, policy_width, data_len=5000, queue=data_queue)
